...
print(bplot)
invisible(dev.off())
# export plot as postscript (change font to Times)
postscript('figures/Fig1.eps', width=4, height=4, horiz=FALSE, family='Times New Roman')
print(bplot)
invisible(dev.off())
```

## Non-cumulative density

```{r plot.both.density.gw.same.panel}
# split by CI type
dplot = ggplot(both.intervals, aes(x=interval, col=factor(itype)))+
  geom_vline(xintercept = 1, lty=1, col='black')+
  geom_density(size=1.1, adjust=1/2)+
  coord_cartesian(xlim=c(0.1, 10))+ # limit x-axis to focus on key change
  scale_x_log10(breaks=c(0.1,0.5,1,2,10), labels=c(0.1,0.5,1,2,10))+
  xlab('Ratio')+
  ylab('Cumulative distribution')+
...
dplot
# export plot
jpeg('figures/DistBothIntervalGWColour.jpg', width=4, height=4, units='in', res=400, quality=100)
print(dplot)
invisible(dev.off())
```
...
invisible(dev.off())
# export for journal
postscript('figures/Fig3.eps',  width=6.5, height=5, horiz=FALSE, family='Times New Roman')
print(lyplot)
invisible(dev.off())
```
...
uplot = ggplot(for.plot, aes(x=interval, col=itype))+
  ...
  ylab('Cumulative distribution')+
  theme_bw()+
  theme(panel.grid.minor = element_blank(), legend.position = c(0.1,0.82))+
...
invisible(dev.off())
# export for journal
postscript('figures/Fig2.eps', width=5, height=3.5, horiz=FALSE, family='Times New Roman')
print(uplot)
invisible(dev.off())
```
